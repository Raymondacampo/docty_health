...
// Three Dots Icon Component (reused from SpecializationMenu)
const ThreeDotsIcon = () => (
...
const DocumentsSection = ({ data, onReload }) => {
    ...
    const menuRefs = useRef({}); // Object to store refs for each document
    ...
    const handleDelete = async (docId) => {
        try {
            ...
            const response = await fetch(`http://localhost:8000/api/auth/delete_document/${docId}/`, {
                method: 'DELETE',
                headers: {
            ...
            onReload(); // Refresh the parent component
            setShowMenu(null);
        } catch (error) {
            console.error('Failed to delete document:', error);
            // Optionally, add user-facing error handling (e.g., alert)
        }
    };
                    ...
                    {data.documents && data.documents.length > 0 ? (
                        data.documents.map((doc) => (
                            ...
                            >
                                ...
                                <div
                                    ref={(el) => (menuRefs.current[doc.id] = el)} // Assign ref for this document
                                    className="relative"
                                >
                                    <button
