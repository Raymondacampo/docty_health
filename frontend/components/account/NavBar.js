...
export default function NavBar({ isOpen, onToggle, selection }) {
  return (
    <div
      className={`h-[90vh] sticky top-0 left-0 bg-white shadow-[3px_4px_6px_-2px_rgba(0,0,0,0.15)] w-[250px] flex-col justify-between 
    ...
    >
      <div>
        <div className="px-2 py-3 text-base text-black flex justify-between items-center font-bold">
          Navigation
          <button
            onClick={onToggle}
            className="lg:block hidden text-[#3d5a80] hover:text-blue-900"
          >
            <svg
        ...
        <a href="/settings" className={`flex items-center gap-2 px-2 py-3 text-sm tracking-wide text-black hover:bg-gray-200 cursor-pointer ${selection == 'acc' && 'bg-gray-200'}`}>Account settings         
        <Cog6ToothIcon className="w-5 h-5 text-[#3D5A80]" strokeWidth={2.5} />
        </a>
        <a href="/doctor_settings" className={`flex items-center gap-2 px-2 py-3 text-sm tracking-wide text-black hover:bg-gray-200 cursor-pointer  ${selection == 'doc' && 'bg-gray-200'}`}>Doctor Account settings
        <div className="flex items-center">
          <Cog6ToothIcon className="w-5 h-5 text-[#3D5A80]"/>            
          <UserIcon className="w-4 h-5 text-[#3D5A80]" />
        </div>

        </a>
        <a href="/availability" className={`flex items-center gap-2 px-2 py-3 text-sm tracking-wide text-black hover:bg-gray-200 cursor-pointer ${selection == 'ava' && 'bg-gray-200'}`}>Availability
        <CheckCircleIcon className="w-5 h-5 text-[#3D5A80]" strokeWidth={2.5} />
        </a>
        <a href="/schedule" className={`flex items-center gap-2 px-2 py-3 text-sm tracking-wide text-black hover:bg-gray-200 cursor-pointer ${selection == 'sch' && 'bg-gray-200'}`}>Schedule management
        <CalendarDateRangeIcon className="w-5 h-5 text-[#3D5A80]" />
        </a>
        <a href="/favorite" className="flex items-center gap-2 px-2 py-3 text-sm tracking-wide text-black hover:bg-gray-200 cursor-pointer">Favorite
        <svg width="15px" height="15px" viewBox="0 0 52 51" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path
        ...
        <a href="/appointments" className="flex items-center gap-2 px-2 py-3 text-sm tracking-wide text-black hover:bg-gray-200 cursor-pointer">Appointments
        <svg width="17px" height="17px" viewBox="0 0 50 51" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path
      ...
      <div className="w-[95%] flex p-2 py-3 ">
        <LogoutButton /> 
      </div>
