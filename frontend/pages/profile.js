...
const ConfigBox = ({title, description, icon, link}) => {
    return(
        <a href={link} class=" border border-[#3d5a80]/25 justify-center items-center  inline-flex hover:bg-[#98c1d1]/25
...
const ConfigValues = ({is_doc}) => {
    return(
        ...
        xs:flex-col xs:w-full xs:gap-y-2">
            ...
            <ConfigBox title={is_doc ? "Recent patients" : "Fav doctors"} description="Configurate your settings right here for your account bla djeh euehr" icon={
...
export default function MyAccountConfig() {
    ...
    useEffect(() => {
      ...
      useEffect(() => {
        console.log(user.email, user.is_doctor)
      },[user])
        ...
        xs:py-16 xs:px-4">
            ...
            xs:w-full">
                <div class="w-full flex-col justify-center items-start gap-2 flex order-first">
                    ...
                    {user.is_doctor ?
                    <div class="text-[#293241] text-xl font-normal font-['Inter']">Hello Dr. {user.email}</div>
                    :
