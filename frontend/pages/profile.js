...
// Helper function to validate token presence and validity
const validateToken = (token) => {
  if (!token) return false; // Missing token
  if (isTokenExpired(token)) return false; // Expired token
...
export default function MyAccountConfig() {
  ...
  useEffect(() => {
    const loadUser = async () => {
      ...
      // Step 1: Check if token is missing or invalid
      if (!validateToken(accessToken)) {
        console.log("Token missing, expired, or invalid. Redirecting to login...");
        router.push("/login");
        return;
      }

      // Step 2: Token is present and not expired, proceed with API call
      try {
        ...
        // Step 3: Handle API errors (e.g., 401 from server)
        if (error.response?.status === 401) {
          console.log("Token rejected by server (possibly blacklisted or invalid). Redirecting to login...");
  ...
  useEffect(() => {
    console.log("User data:", user.email, user.is_doctor);
  }, [user]);
  ...
  return (
    <div
      className="w-full bg-[#98c1d1]/20 flex-col justify-center items-center gap-16 inline-flex
        md:px-9
        sm:py-24
        xs:py-16 xs:px-4"
    >
      <div
        className="flex-col justify-center items-center gap-11 flex 
        sm:w-auto
        xs:w-full"
      >
        <div className="w-full flex-col justify-center items-start gap-2 flex order-first">
          <div className="text-[#293241] text-xl font-bold font-['Inter']">
            My account
          </div>
          {user.is_doctor ? (
            <div className="text-[#293241] text-xl font-normal font-['Inter']">
              Hello Dr. {user.email}
            </div>
          ) : (
            <div className="text-[#293241] text-xl font-normal font-['Inter']">
...
const ConfigBox = ({ title, description, icon, link }) => {
  return (
    <a
      href={link}
      className="border border-[#3d5a80]/25 justify-center items-center inline-flex hover:bg-[#98c1d1]/25
        sm:flex-col sm:w-[300px] sm:order-first sm:gap-4 sm:px-8
        xs:flex-row xs:w-full xs:order-last xs:py-4 xs:gap-4 xs:px-4"
    >
      <div
        className="flex-col justify-center items-start flex
        sm:gap-4
        xs:gap-2"
      >
        <div
          className="text-[#293241] font-bold font-['Inter']
          sm:text-lg xs:text-base"
        >
          {title}
        </div>
        <div
          className="text-[#293241] font-medium font-['Inter']
      ...
      <div
        data-svg-wrapper
        className="sm:order-last xs:order-first xs:max-h-[50px] xs:max-w-[50px]"
      >
        {icon}
      </div>
    </a>
...
const ConfigValues = ({ is_doc }) => {
  return (
    <div
      className="flex flex-wrap justify-center gap-x-12
