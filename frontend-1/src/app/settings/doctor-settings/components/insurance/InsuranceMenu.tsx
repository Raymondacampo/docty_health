...
export const InsuranceMenu: React.FC<InsuranceMenuProps> = ({ Insurance, onDelete }) => {
  ...
  const handleDelete = async () => {
    try {
      await apiClient.delete(`/doctors/remove_insurance/${Insurance.id}/`);
      onDelete({ msg: `Insurance ${Insurance.name} deleted successfully`, status: 'success' });
      setIsMenuOpen(false);
    } catch (error) {
      ...
      {isMenuOpen && (
        <div className="absolute right-0 mt-2 w-32 bg-white border border-gray-300 rounded shadow-lg z-10">
          <button
            ...
            aria-label={`Delete Insurance ${Insurance.name}`}
          >
            Delete
          </button>
        </div>
