...
const StarBar = ({ star, count, totalReviews }:{ star: number; count: number; totalReviews: number; }) => {
    ...
    <div className="w-full justify-start items-center gap-4 inline-flex">
      <div className="w-[95%] justify-start items-center gap-4 flex">
        <div className="flex items-center gap-2">
          <span className="w-[60px] text-black text-sm tracking-wide">
            {star} star{star !== 1 ? "s" : ""}
          </span>
        </div>
      ...
      <div className="text-center text-gray-500 text-xs font-normal tracking-wide text-nowrap">
        {count} reviews
      </div>
    </div>
  );
...
const TotalRating = ({ averageRating, reviewCount, ratingDistribution }: { averageRating: number; reviewCount: number; ratingDistribution: Record<number, number>; }) => {
  return (
    <div className="w-full p-4 bg-white rounded-[10px] shadow-[0px_4px_4px_0px_rgba(0,0,0,0.15)] flex-col justify-start items-start gap-6 inline-flex lg:min-w-[450px] lg:max-w-[450px]">
      <div className="w-full justify-start gap-2 inline-flex flex-col">
        ...
        <div className="justify-start items-center gap-4 flex flex-row">
          <div className="text-black text-lg tracking-wide flex gap-2">
            <AverageRatingStars averageRating={averageRating} />
            {averageRating ? averageRating.toFixed(1) : "0.0"} out of 5
          </div>
        </div>
        <div className="text-black tracking-wide text-xs">
